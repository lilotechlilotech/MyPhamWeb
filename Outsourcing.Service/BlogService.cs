...
namespace Outsourcing.Service
{
    ...
    public class BlogService : IBlogService
    {
        ...
        {
            var blogs = this.GetBlogs().Where(p => p.BlogCategoryId == 3 && p.IsAvailable == true && p.Deleted == false).OrderBy(p=>p.DateCreated).Take(3);
            return blogs;
        }
        #endregion
        ...
        {
            return blogRepository.GetAll().Where(p => p.BlogCategoryId == 3).OrderByDescending(p => p.DateCreated);
        }
    }
}
